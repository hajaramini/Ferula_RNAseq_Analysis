...
dim(DE_Genes_CPM) #3823  12

library(ggdendro)
#install.packages("pvclust")
library(pvclust)
pdf(file = "~/Kallisto_input_out/heatmap_DE_Genes_CPM.pdf",width = 20, height = 8)
hc.mds.dge <- hclust(dist(t(DE_Genes_CPM)))
plot(hc.mds.dge)
rect.hclust(hc.mds.dge, k = 4, border = "red")
heatmap.2(as.matrix(DE_Genes_CPM), Rowv=as.dendrogram(hc.mds.dge), scale="row", density.info="none", trace="none")
dev.off()
d<-as.matrix(DE_Genes_CPM)
pdf(file ="~/Kallisto_input_out/Heatmap_DE_Genes_CPM.pdf", height=10,width=10)
...
```{r}
colnames(DEgenes2)[1] <- "GeneID"
DE_counts <- DEgenes2 %>% select(GeneID)
#get count data specifically for the GxE genes
