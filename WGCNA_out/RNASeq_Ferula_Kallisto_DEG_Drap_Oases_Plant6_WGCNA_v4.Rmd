...
counts_WGCNA <-counts_WGCNA[rowSums(counts_WGCNA > 10) >= 3,]
...
nGenes <- ncol(datExpr0root) #22995
nSamples
...
nGenes <- ncol(multiExpr[[3]]$data) #22995
...
nGenes <- ncol(multiExpr[[4]]$data) #22995
...
nGenes <- ncol(multiExpr[[5]]$data) #22995
nSamples
...
sizeGrWindow(8,10)
pdf(file = "~/Ferula_RNAseq_Rbased/WGCNA_out/Plot_v4/GeneSignificanceTraits_v2.pdf", width = 8, height = 10);
#par(mfrow = c(6,1))
...
mkdir Post_WGCNA_Analysis, then cp muduleNumber.txt to find each DEgenes's module number