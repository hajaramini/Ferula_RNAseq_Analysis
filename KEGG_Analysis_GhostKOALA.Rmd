...
head(Ferula_cdna)
bias<-nchar(Ferula_cdna)
names(bias)<-names(Ferula_cdna)
length(bias) # 60134
#  bias.data vector must have the same length as DEgenes vector!
GOseq.Ferula.ORA2<-function(genelist,custom.category.list=Ferula.KEGG.list, padjust=0.05,ontology="KEGG") { # return GO enrichment table, padjus, pjust=0.05, ontology (either "BP","CC","MF", or "KEGG"), custom.category.list is either Brgo.list (for GO), Ferula.KEGG.list (for KEGG), or 
    TF<-(names(bias) %in% genelist)*1
    names(TF)<-names(bias)
    #print(TF)
    pwf<-nullp(TF,bias.data=bias)
    #print(pwf$DEgenes)
    GO.pval <- goseq(pwf,gene2cat=custom.category.list,use_genes_without_cat=TRUE) 
    #GO.pval <- goseq(pwf,gene2cat=Brgo.DF3,use_genes_without_cat=TRUE) 
    #head(GO.pval) 
  if(ontology=="BP") {
...
# test
GOseq.Ferula.ORA2(ko00010.genelist) # should have "ko00010". OK worked.
```
