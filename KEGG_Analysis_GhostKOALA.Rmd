...
GOseq.Ferula.ORA2<-function(genelist,custom.category.list=Ferula.KEGG.list, padjust=0.05,ontology="KEGG", bias =Ferula_bias) { # return GO enrichment table, padjus, pjust=0.05, ontology (either "BP","CC","MF", or "KEGG"), custom.category.list is either Brgo.list (for GO), Ferula.KEGG.list (for KEGG), or 
    Ferula_bias.KEGG<-bias[names(bias) %in% names(Ferula.KEGG.list)]
    TF<-(names(Ferula_bias) %in% genelist)*1
